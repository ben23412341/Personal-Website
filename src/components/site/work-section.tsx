...
/** How sharply the panel wipes as a node leaves the right-hand slot. */
...
export function WorkSection() {
  ...
  React.useEffect(() => {
    ...
    const observer = new ResizeObserver(([entry]) => {
      setRadius(entry.contentRect.width * 0.37);
    });
    observer.observe(el);
  ...
  /** Bring a project round to the right-hand slot by the shortest path. */
  ...
  const nodes = projects.map((project, index) => {
    ...
    const delta = normalize(angle);
    // 1 when the node is at the right-hand side, 0 when it is at the far left.
    ...
    <section id="work" className="relative bg-black">
      ...
      <div className="px-5 pb-20 pt-10 sm:px-8 sm:pb-28 sm:pt-14">
        ...
        <div className="relative">
          <span
            aria-hidden="true"
            className="pointer-events-none absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 whitespace-nowrap font-display text-[13vw] leading-none tracking-tight text-white/[0.05]"
          >
            Work
          </span>
        ...
        <div className="mt-12 flex flex-col items-center justify-center gap-12 lg:flex-row lg:gap-16">
          {/* Orbit */}
          <div className="relative w-full max-w-[min(34rem,88vw)]">
            <div
              ref={orbitRef}
            ...
            >
              {/* Orbit rings */}
              <div className="pointer-events-none absolute inset-[13%] rounded-full border border-white/12" />
              <div className="pointer-events-none absolute inset-[30%] rounded-full border border-white/6" />
              ...
              {nodes.map(({ project, index, x, y, proximity }) => {
                ...
                const visual = (
                  <>
                    ...
                    <span
                      className={cn(
                        "absolute left-1/2 top-[calc(100%+0.6rem)] w-36 -translate-x-1/2 text-balance text-center font-mono text-[11px] uppercase leading-tight tracking-[0.18em] transition-colors duration-300",
                ...
                >
                  <span
                    className={cn(
                      ...
                      autoRotate ? "left-4 bg-black" : "left-1 bg-white/60",
                    )}
                  />
                </span>
                <span
                  className={cn(
                    ...
                    autoRotate ? "text-white/70" : "text-white/40",
                  )}
                >
                  auto-rotating
                </span>
          ...
          {/* Panel for whichever project has come round to the right */}
          <div className="relative w-full max-w-[38rem]">
            ...
            <Link
              ...
              style={{
                // Wipes away right-to-left, then the next one is revealed
                // left-to-right — both anchored at the panel's left edge.
                clipPath: `inset(0 ${(1 - openness) * 100}% 0 0)`,
                transform: `translateX(${(1 - openness) * -24}px)`,
              }}
            >
            ...
            {/* The wipe front, drawn as a hairline — the same 1px rule the
                rest of the page is built from, here used as a scan edge. */}
            <div
              aria-hidden="true"
              className="pointer-events-none absolute -bottom-2 -top-2 w-px bg-white transition-opacity duration-200"
              style={{
